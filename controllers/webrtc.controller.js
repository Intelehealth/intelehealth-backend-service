...
function getRoomParameters(req, roomId, clientId, isInitiator) {
  ...
  */
  ...
  if (req.query["minre"] || req.query["maxre"]) {
    var message =
      'The "minre" and "maxre" parameters are no longer supported. Use "video" instead.';
    console.error(message);
    errorMessages.push(message);
  }
  ...
  var mediaConstraints = makeMediaStreamConstraints(
    audio,
    video,
    firefoxFakeDevice
  );
