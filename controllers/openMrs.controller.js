...
const getVisitCountQueryWithLocation = ({
...
(select 	v.visit_id,
  ...
  group by 	v.visit_id,
        v.patient_id
  having max(case
        when attribute_type_id = 5 then value_reference else null end) = "${speciality}") as t2
...
and state = "${location}"
group by speciality,state,case
    when  (encounter_type = 14 or encounter_type = 12  or com_enc = 1) then "Completed Visit"
          when  (encounter_type = 9 ) then "Visit In Progress"
          when (encounter_type) = 15 then "Priority"
          when  ((encounter_type = 1 or encounter_type = 6) )  then "Awaiting Consult"
...
const getVisitCounts = async (req, res, next) => {
  ...
  const query =
    location === "All"
      ...
      : getVisitCountQueryWithLocation({ speciality, location });
