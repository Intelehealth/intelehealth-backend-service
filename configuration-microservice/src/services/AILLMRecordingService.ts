...
async function getAll(): Promise<any> {
  const aillm = await AILLMRecording.findAll({ attributes: ["id", "key", "name", "is_enabled", "createdAt", "updatedAt"], raw: true });
  return aillm;
}
...
async function updateIsEnabled(id: string, is_enabled: boolean, user_id: string, user_name: string): Promise<void> {
  ...
  await AuditTrail.create({user_id, user_name, activity_type: "AI LLM RECORDING CONFIG UPDATED", description: `${is_enabled ? "Enabled" : "Disabled"} "${aillmRecording.key}" config.`});
}
