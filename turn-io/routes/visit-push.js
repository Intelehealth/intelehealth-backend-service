...
//       // Show which top-level keys Turn actually sent and how the symptom detail
//       // rows resolved -- makes key-name mismatches (e.g. skip_rash_* vs the
//       // field-list keys) obvious without digging through the full bundle.
//       console.log("[visit_push] parsed:", JSON.stringify({
//          patient_uuid: personUuid,
//          symptom,
//          symptoms_data_keys: symptomData ? Object.keys(symptomData) : null,
//          has_results: !isBlank(results) && Object.keys(results).length > 0,
//       }));
...
router.post("/visit_push", async (req, res) => {
   ...
   try {
      ...
      const answers = req.body.answers || req.body.symptoms_data || req.body.results || {};
      const patientHistory = req.body.patient_history || {};
      const familyHistory = req.body.family_history || {};
